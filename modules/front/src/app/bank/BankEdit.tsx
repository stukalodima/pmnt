...
import { PATH, NEW_SUBPATH } from "./BankManagement";
...
import "../../app/App.css";
import { Bank } from "../../cuba/entities/finance_Bank";

type Props = {
  entityId: string;
};

type BankEditAssociationOptions = {};

type BankEditLocalStore = BankEditAssociationOptions & {
  updated: boolean;
  globalErrors: string[];
  formRef: RefObject<FormInstance>;
};

const FIELDS = ["name", "mfo"];
...
const getAssociationOptions = (
  mainStore: MainStore
): BankEditAssociationOptions => {
  const { getAttributePermission } = mainStore.security;
  const associationOptions: BankEditAssociationOptions = {};

  return associationOptions;
};

const BankEdit = (props: Props) => {
  ...
  const dataInstance = useInstance<Bank>(Bank.NAME, {
    view: "_local",
    loadImmediately: false
  });

  const store: BankEditLocalStore = useLocalStore(() => ({
    // Association options
  ...
  useEffect(() => {
    if (isNewEntity(entityId)) {
      dataInstance.current.setItem(new Bank());
    } else {
